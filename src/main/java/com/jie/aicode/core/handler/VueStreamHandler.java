...
import com.jie.aicode.ai.model.message.*;
import com.jie.aicode.ai.tools.BaseTool;
import com.jie.aicode.ai.tools.ToolManager;
    ...
    public Flux<String> handle(Flux<String> originFlux,
        ...
        return originFlux
                ...
                .doOnComplete(() -> {
                    ...
                    String path = AppConstant.CODE_DIR + "/vue_" + appId;
                    // 异步构建项目
                    VueBuilder.buildProjectAsync(path);
                })
                .doOnError(error -> {
                    // 如果AI回复失败，也要记录错误消息
    ...
    private String handleJsonMessageChunk(String chunk, StringBuilder chatHistoryStringBuilder) {
        ...
        switch (typeEnum) {
            ...
            case TOOL_REQUEST -> {
                ...
                // 检查是否是第一次看到这个工具 ID
                if (toolId != null && !set.contains(toolId)) {
                    set.add(toolId);
                    // 根据工具名称获取工具实例
