...
//    private AiCodeTypeChoiceService aiCodeTypeChoiceService;
    ...
    public void downloadAppCode(@PathVariable Long appId,
        ...
        String sourceDirPath = AppConstant.CODE_DIR + File.separator + sourceDirName;
        // 5. 检查代码目录是否存在
    ...
    public Flux<ServerSentEvent<String>> createCode(@RequestParam Long appId,
        ...
        if (StrUtil.isBlank(message)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "message错误");
        }
        // 获取当前登录用户
        User loginUser = userService.getLoginUser(request);
        Flux<String> codeFlux = appService.createCode(appId, message, loginUser);
        return codeFlux
                .map(code -> {
                    //防止空格读取丢失
                    Map<String, String> data = Map.of("d", code);
